...
jobs:
  test:
    ...
    strategy:
      matrix:
        os: [ubuntu-latest, windows-latest] # region: [us-east-1, us-west-2, ap-south-1],  browser: [chrome, firefox]
    steps:
      - name: Print OS
...
```
Real-Time Devops GithubActions Yaml

```

name: Java Matrix Testing
...
jobs:
  test:
    ...
    strategy:
      matrix:
        os: [ubuntu-latest, windows-latest, macos-latest] #  region: [us-east-1, us-west-2, ap-south-1], browser: [chrome, firefox]
    steps:
...
jobs:
  deploy:
    ...
    strategy:
      matrix:
        os: [ubuntu-latest, ubuntu-20.04, windows-latest] # region: [us-east-1, us-west-2, ap-south-1], browser: [chrome, firefox]
        images: [hello-world, alpine]
    steps:
      - name: Docker details
        run: docker info
...
## Use Case 7 — Using matrix for service containers (database)
